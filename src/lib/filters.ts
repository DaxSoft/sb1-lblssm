...
export const filterSchema = z.object({
  operator: z.enum([...filterOperators.string, ...filterOperators.number, ...filterOperators.date]),
  value: z.unknown(),
...
export function applyFilter(value: unknown, filter: FilterValue, type: 'string' | 'number' | 'date'): boolean {
  ...
  switch (type) {
    ...
    case 'date':
      return applyDateFilter(value instanceof Date ? value : new Date(value as string), filter);
    default:
      return true;
  }
}
...
function applyDateFilter(value: Date, filter: FilterValue): boolean {
  ...
  const filterValueTo = filter.valueTo ? new Date(filter.valueTo as string) : undefined;
