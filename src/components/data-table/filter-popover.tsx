...
export function FilterPopover<TData>({
  ...
  const [localFilter, setLocalFilter] = useState<FilterValue | null>(filterValue);
  const columnType = column.columnDef.meta?.type as 'string' | 'number' | 'date' || 'string';

  const handleApplyFilter = useCallback(() => {
    if (!localFilter?.operator || (!localFilter?.value && localFilter?.value !== 0)) {
      handleClearFilter();
      return;
    }
        ...
        return (
          <div className="grid gap-2">
            <div className="flex flex-col gap-2">
              ...
              <Calendar
                mode="single"
                selected={localFilter.value ? new Date(localFilter.value as string) : undefined}
                onSelect={(date) =>
                  setLocalFilter({
                    ...localFilter,
            ...
            {localFilter.operator === 'between' && (
              <div className="flex flex-col gap-2">
                ...
                <Calendar
                  mode="single"
                  selected={localFilter.valueTo ? new Date(localFilter.valueTo as string) : undefined}
                  onSelect={(date) =>
                    setLocalFilter({
                      ...localFilter,
