...
export function ExportDialog<TData>({
  ...
  const handleExport = async () => {
    try {
      setIsExporting(true);
      await exportToExcel(table, 'users', Object.keys(selectedFields).filter(key => selectedFields[key]));
      // Save preferences
        ...
        <div className="space-y-4 py-4">
          ...
          {Object.values(selectedFields).some(Boolean) && (
            <div className="rounded-md bg-muted p-4">
              <h4 className="mb-2 text-sm font-medium">Selected Fields Preview</h4>
